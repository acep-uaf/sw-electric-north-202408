...
from functions.api_xlsx import xlsx_url_to_file
...
xlsx_url_to_file(
    ...
    file = 'data/raw/es/data/Energy_Stats_Infrastructure_2021.xlsx')
...
gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.longitude, df.Latitude), crs='EPSG:4326').to_crs(3338).to_file(
    'data/raw/es/data/es_lookup_plants.geojson', driver='GeoJSON')
...
xlsx_url_to_file(
    ...
    file = 'data/raw/es/data/Energy_Stats_Generation_Tables.xlsx')


# pull sales reporting data from generation xlsx, save to file
df = pd.read_excel(
        'data/raw/es/data/Energy_Stats_Generation_Tables.xlsx',
...
(gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.Longitude, df.Latitude), crs='EPSG:4326')
    .to_crs(3338)
    .to_file('data/raw/es/data/es_lookup_sales_report.geojson', driver='GeoJSON'))
...
df.to_csv('data/raw/es/data/es_annual_operations.csv', index=False)
...
df.to_csv('data/raw/es/data/es_monthly_gen.csv', index=False)
...
df.to_csv('data/raw/es/data/es_gen_fuel_type.csv', index=False)
