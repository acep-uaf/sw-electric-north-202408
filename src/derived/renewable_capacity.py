...
infrastructure = pd.read_csv('data/derived/infrastructure.csv')
...
sum_capacity = infrastructure.groupby(['intertie_id'])['capacity_mw'].sum().reset_index(
...
renewable_capacity = infrastructure.loc[
    ...
    ['intertie_id', 'capacity_mw']
].groupby('intertie_id')['capacity_mw'].sum().reset_index(
...
renewable_ratio['ratio_renewable_capacity'] = renewable_ratio['renewable_capacity_mw']/renewable_ratio['sum_capacity_mw']

out = renewable_ratio

out.to_csv('data/derived/renewable_capacity.csv', index=False)
